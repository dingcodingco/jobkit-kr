...
`data/pipeline.md`에 쌓인 채용 공고 URL을 처리합니다. 후보자는 원할 때 URL을 추가하고, 이후 `/jobkit pipeline`을 실행해 한 번에 처리합니다.
...
2. **각 대기 URL에 대해**:
   ...
   d. **전체 auto-pipeline 실행**: Evaluation A-G -> Report .md -> PDF(score >= 3.0이면) -> Tracker
