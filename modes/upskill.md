...
- **Scope boundary:** the plan LINKS each resource to `/jobkit training {name}` for a full judging pass; it never runs training's 6-dimension scoring itself. `upskill` finds; `training` judges.
...
→ To judge one of these against your profile, run `/jobkit training {resource name}`.
...
8. **Scope boundary.** Plan entries link to `/jobkit training {name}` for judging a specific resource; the plan itself never runs training's 6-dimension scoring. `upskill` finds; `training` judges.
