...
1. `/jobkit email {report-number-or-slug}`
   ...
   - If `data/pdf-index.tsv` contains a row for that report, use that same row's
     ...
     should be generated first via `/jobkit pdf {slug}` or attached
     manually.

2. `/jobkit email {pasted JD}`
...
4. `/jobkit email stuck {report-number-or-slug}`
...
5. `/jobkit email noshow {report-number-or-slug}`
   - Load the matching `reports/{NNN}-*.md` for company and role context.
