...
1. `/jobkit offer-prep {pasted contract text}`
2. `/jobkit offer-prep {path to PDF or file}` — e.g. a contract dropped
   into `data/offers/{company-slug}/`
3. `/jobkit offer-prep` — ask for the document
4. Proactively: when a tracker row is being set to `Offer`, suggest this mode.
5. `/jobkit offer-prep reply {company-slug}` — Step 8 on demand: draft
