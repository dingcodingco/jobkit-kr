...
> "No active applications to follow up on. Apply to some roles first with `/jobkit` and come back when they're aging."
...
- Suggest the user run `/jobkit contacto {company}` to find the right person first
...
> - Trying a different contact via `/jobkit contacto`
...
**To:** {email or "No contact found — run `/jobkit contacto` first"}
