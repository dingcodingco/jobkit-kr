...
用于批量处理记录在 `data/pipeline.md` 中的职位 URL 链接。候选人可以随时向待处理列表中追加抓取的岗位链接，然后运行 `/jobkit pipeline` 一键自动化评估所有待投递机会。
...
2. **循环遍历处理**：对每一个未处理的 URL：
   ...
   **关于自动生成 PDF 的分流控制 (Configurable)：**
   ...
   如果本次岗位的评估得分低于该阈值，系统将**跳过**简历 PDF 的生成步骤（照常撰写评估报告，并在报告头部标明 `**PDF:** not generated — run /jobkit pdf {company-slug} to create on demand`，同时在 tracker 的 PDF 列登记为 ❌）。
