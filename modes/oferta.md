...
If deeper company research is useful, recommend running `/jobkit deep` separately after the evaluation.
...
After saving the report and recording in the tracker, append a cover letter draft to the report file under `## Cover Letter Draft`. This is a starting point — not the final letter. The user completes it via `/jobkit cover {slug}`.
...
> Draft generated at evaluation time. Complete via `/jobkit cover {slug}` to fill in angles, confirm research, and generate the PDF.
...
---
*Run `/jobkit cover {slug}` to complete angles, confirm company research, and generate the PDF.*
```
