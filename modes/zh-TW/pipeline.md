...
用於批次處理記錄在 `data/pipeline.md` 中的職缺 URL。求職者可以隨時把看到的職缺連結丟進待處理清單，之後執行 `/jobkit pipeline` 一次自動評估所有機會。
...
2. **逐一處理**：對每一個未處理的 URL：
   ...
   **自動產生 PDF 的分流控制（可設定）：**
   ...
   若本次職缺的評分低於門檻，系統會**跳過**履歷 PDF 的產生步驟（照常撰寫評估報告，並在報告開頭標明 `**PDF:** not generated — run /jobkit pdf {company-slug} to create on demand`，同時在 tracker 的 PDF 欄登記為 ❌）。
