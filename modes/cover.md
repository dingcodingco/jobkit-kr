...
Works in two modes:
- **Slug mode:** `/jobkit cover {slug}` — loads the existing evaluation report draft as a starting point
- **Paste mode:** `/jobkit cover` or JD pasted directly — starts from scratch
...
When invoked as `/jobkit cover {slug}`:
