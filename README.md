...
|---|---|---|
| 한국 공고 수집 | 없음 | 원티드·점핏·그리팅 (공개 페이지, 인증 불필요) |
...
**그리팅** (`provider: greeting`) — 국내에서 가장 널리 쓰이는 채용 관리 시스템이다.
회사마다 주소가 따로 있어서(`{회사}.career.greetinghr.com`) 가고 싶은 회사를 직접
적어 준다. 상위 도구가 Greenhouse·Lever를 회사 단위로 읽는 방식과 같다.
...
**사람인**은 공개 오픈API가 있는데 **인증키가 필요하다.** 이 포크가 기본으로
붙이는 소스는 키 없이 열리는 것들이라 여기엔 안 넣어 뒀다. 키를 받아서 쓰고
싶으면 [사람인 오픈API](https://oapi.saramin.co.kr/guide)에서 발급받아
직접 소스를 늘리면 된다. 어댑터 만드는 법은
[providers/ADDING_A_PROVIDER.md](providers/ADDING_A_PROVIDER.md)에 있다.
