...
1. In a terminal (one time), clone career-ops **and install its dependencies** — Cowork's local shell has no npm network access, so do this before opening the folder:
   ```bash
   git clone https://github.com/career-ops-hq/career-ops.git ~/jobkit
   cd ~/jobkit && npm install
   ```
2. Install [Claude Cowork](https://claude.com/download) and, in **Colaborar/Collaborate** mode, add the `~/jobkit` folder.
